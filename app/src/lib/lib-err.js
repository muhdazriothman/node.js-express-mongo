...
// const i18n = require('@require('@newspage/lib-commons/i18n');');
const fs = require('fs');
...
exports.error = (err, req, res, next) => {
  ...
  res.format({
    html: () =>
      res.send(
        `<!doctype html><html lang="en"><head><title>${res.statusCode} ${error.code}</title></head><body>${res.statusCode} (${error.code}) &mdash; ${
          error.msg
...
async function initialize() {
  let errorFilePath = config('errors:path');
  if (!errorFilePath) {
...
function registerError(error) {
  ...
  function ClientError() {
    ...
    // let msg = message != null ? message : i18n.__(error.message);
